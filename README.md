...
如果图片宽度或高度超过上限, 会自动缩小图片, 如果缩小后仍超过体积上限,
会报错. 如果图片虽然未超过宽度/高度上限, 但体积超过上限, 则直接报错.

### 缩略图

在 gallery.toml 中有以下两项设定:

- **thumb_size**: 缩略图边长 (缩略图总是正方形)
- **image_output_format**: 缩小图片或生成缩略图时, 输出的图片格式

其中 image_output_format 可选 JPEG 或 WebP, 默认是 WebP,
该参数对图片大小超过上限时的自动缩小及生成缩略图有效.
